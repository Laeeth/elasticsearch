...
int main(string[] args)
{
	...
	ApiCall[] apiCalls;
	auto sources = dirEntries(".","*.json",SpanMode.depth);
	foreach(source; sources)
	{
		auto json = parseJson(source.name);
		apiCalls ~= parseAPI(json);
	}
...
enum CodePrelude =
q{
module kaleidic.sil.std.extra.elasticsearch;
...
shared string elasticSearchAPIURL = "https://api.trello.com";
shared string elasticSecret, elasticAuth;
...
void registerElasticsearch(ref Handlers handlers)
{
	import std.meta:AliasSeq;
	handlers.openModule("elasticsearch");
...
@SILdoc("set Elasticsearch secrets from ELASTIC_SECRET and ELASTIC_AUTH environmental variables")
string setSecrets()
{
	import std.process: environment;
	import std.exception: enforce;
	elasticSecret = environment.get("ELASTIC_API_KEY","");
	enforce(trelloSecret.length > 0, "ELASTIC_API_KEY environmental variable must be set");
	elasticAuth = environment.get("ELASTIC_AUTH","");
	enforce(elasticAuth.length > 0, "ELASTIC_AUTH environmental variable must be set");
	return "Secret has been set to ELASTIC_API_KEY and auth to ELASTIC_AUTH environment variables";
}
